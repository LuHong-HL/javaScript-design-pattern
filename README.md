...
### 前言
纯属是为了敲一遍代码而写，觉得有帮助大佬们可以给个赞喔。后面，有提供github链接以及此内容的书籍。
...
**说明：** 迭代器模式可以把迭代的过程从业务逻辑中分离出来，在使用迭代器模式之后，即使不关心对象的内部构造，也可以按顺序访问其中的每个元素。迭代器模式是一种相对简单的模式，简单到很多时候我们都不认为它是一种设计模式。目前 的绝大部分语言都内置了迭代器。我们平时用的`Array.property.forEach()`，`for...in...`，`for...of...`这些都是迭代器。
...
```
- **项目仓库地址：** [javaScript-design-pattern](https://github.com/LuHong-HL/javaScript-design-pattern)
